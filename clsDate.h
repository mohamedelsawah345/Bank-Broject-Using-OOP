...
class clsDate
{
	...
	public:
		...
		static string GetSystemTimeTostring()
		{
			time_t TT = time(0);
			tm*NOW= localtime(&TT);
			short Day, Month, Years, Hours, Minuts, Second;
			Day = NOW->tm_wday;
			Month = NOW->tm_mon+1;
			...
			return to_string(Day) + ": " + to_string(Month) + ": " + to_string(Years) + "_ " +
				to_string(Hours) + ": " + to_string(Minuts) + " :" + to_string(Second) + " ";
